...
/// <summary>
/// 心情報(HeartProfile)から 形/色/半径 を決定論的に割り当てるマッパー。
/// 値域や分布の偏りに合わせてガンマで均し、見た目が“そこそこ一様”にばらけるよう調整。
/// </summary>
public class HeartAppearanceMapper : MonoBehaviour
{
    ...
    [Tooltip("Spawn 時に radius を上書きするか（他の成長システムがあるならOFF推奨）")]
    public bool setRadiusOnSpawn = true;

    [Header("Shape")]
    [Tooltip("シーンに存在する ShapeType の総数（HeartVisualの列挙数に合わせる）")]
    public int shapeCount = 3; // 例: 0=Circle,1=Triangle,2=Box
    ...
    public void Apply(HeartProfile hp, HeartVisual vis)
    {
        ...
        // 2) 形（準一様化して 0..1 → 0..shapeCount-1）
        //    互いに無理数っぽい係数で線形結合 → frac で包絡（値域偏りでも程よく散る）
